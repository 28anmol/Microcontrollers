...
{
	// TODO
	return (( in >> 4 ) * 10) + (in & 15);     // Coverts bcd to binary system.
}
/******************************************************************************/
...
uint8_t ds1307_getTime(void)
{
	...
	// Read value
	second = ds1307_decodeBcd(i2c_master_read_next() & 0x7F);   // Binary format 0b01111111  -- Hexa = 0x7F      // All values from the given table and setting
	// TODO minute, hour, ...																						up the required bits.
	minute = ds1307_decodeBcd(i2c_master_read_next() & 0x7F);	// Binary format 0b01111111  -- Hexa = 0x7F
	hour = ds1307_decodeBcd(i2c_master_read_next() & 0x3F);		// Binary format 0b00111111  -- Hexa = 0x3F
	weekday = ds1307_decodeBcd(i2c_master_read_next() & 0x07);	// Binary format 0b00000111	 -- Hexa = 0x07
	day = ds1307_decodeBcd(i2c_master_read_next() & 0x3F);		// Binary format 0b00111111  -- Hexa = 0x3F
	month = ds1307_decodeBcd(i2c_master_read_next() & 0x1F);	// Binary format 0b00011111  -- Hexa = 0x1F
	year = ds1307_decodeBcd(i2c_master_read_last() & 0xFF);		// Binary format 0b11111111  -- Hexa = 0xFF
	// Close I2C bus
	i2c_master_close();
